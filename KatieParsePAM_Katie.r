...
#      out.reg[i,"Run"] <- i
   ...
   out.rlc <- as.data.frame(matrix(NA,nrow=length(n.slcs)*9,ncol=length(colnms)+3))
   colnames(out.rlc) <- c("Run","LN","Mem",colnms) #Note: Fm_l = Fm' and Fo_l = Fo'
   out.rlc$Run <- rep(1:length(n.slcs),each=9) #may have runs terminated early
   ...
   for (i in 1:length(n.slcs)) {
      ...
      for (j in 1:9) {
         ...
         if ((n.rlc+j < n.slce[i]) && (length(grep("\"FO\"",pamdat[n.rlc+j],fixed=T))>0 |
            length(grep("\"F\"",pamdat[n.rlc+j],fixed=T))>0)) {
...
#   out.rlc[out.rlc=="-"] <- NA #PAM uses '-' for NA, might need to replace '-' entries
   ...
   colnames(out.rlc)[4:ncol(out.rlc)] <- colnms
   
   #link light curve with regression parameters (if available!)
   ...
   #add memory line number for reference
